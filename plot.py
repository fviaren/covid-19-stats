...
def plot_countries_dataframe(
    df, countries=list, last_date=datetime, days_number=int
):
    ...
    fig, axes = plt.subplots(
        nrow, ncol, sharex=True, gridspec_kw={"hspace": 0}
    )
    ...
    for row_position in range(nrow):
        ...
        curr_df.unstack().plot(
            kind=plot_kind, ax=axes[row_position], legend=None
        )
