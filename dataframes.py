...
def create_countries_df(data=dict) -> "pandas.core.frame.DataFrame":
    df = (
        pd.DataFrame(data)
        .transpose()
        .sort_values(by=["Total Confirmed Cases"], ascending=False)
    )
...
def create_df_all_countries_daily_data(data=list) -> "pd.core.frame.DataFrame":
    df = pd.DataFrame(data).drop(
        columns=["Lon", "Lat", "CountryCode", "Province", "City", "CityCode"]
    )
    return df


def pivot_df(df, columns):
    pivoted = df.pivot(index="Date", columns=columns)
    return pivoted
